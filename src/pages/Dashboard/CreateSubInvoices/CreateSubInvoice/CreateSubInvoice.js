...
import styles from "./CreateSubInvoice.module.css";
...
const CreateInvoice = ({ jobId, clientId }) => {
  const { id } = useParams();
  ...
  const handleInvoice = async (e) => {
    ...
    try {
      ...
      // resetForm();
      setTimeout(() => {
        navigate(`/dashboard/subinvoices/${id}`);
      }, 3000);
    } catch (error) {
  ...
  return (
    <div className={styles.create__invoices}>
      ...
      <div className={styles.invoices__btns}>
        ...
        {loading ? (
          <button disabled={loading} style={{ opacity: "0.8" }}>
            <img src={save} alt="" />
            <span>Saving...</span>
          </button>
        ) : (
